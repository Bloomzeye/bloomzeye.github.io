---
title: "blog coming soon...ish!"
# image: 
#   path: assets\images\backgrounds\cloudsBG.png
#   thumbnail: assets\images\backgrounds\cloudsBG.png
#   caption: "pixel art cloud background"
categories: annoucement
tags: annoucement pinned
permalink: /coming-soon/
# pinnedImg: /blog/assets\images\backgrounds\cloudsBG.png
# pinnedImgAlt: "pixel art cloud background"
excerpt: "I learned Jekyll, a static site builder, after seeing (1) video one it...and have proceeded to spend about 2 weeks learning how to code my own blog from it. #justAuDHDthings"
---

## Check out my [about me page here]({% link about.markdown %}) for all the details and info on my blog!

I learned Jekyll, a static site builder, after seeing (1) video one it...and have proceeded to spend about 2 weeks learning how to code my own blog from it. #justAuDHDthings

Since I'm a lil' impatient, I'm deploying my site now, because I NEED to have this task finished for my brain to leave it alone.

You're going to see a lot of my code being a little wonky here...hence learning a new programming syntax within 2 weeks.

Oh, and spelling errors, since I can't seem to bring myself to use a spellchecker in VSCodium right now.

Thanks for being here, and check back within another 2 weeks once my brain settles down!
<hr>
<h3 class="aboutSubHeader">find me on the interwbs!</h3>
<ul class="links" style="list-style: none;">
    <li><i class='fas fa-bug'></i> <a href="https://bsky.app/profile/bloomzeye.bsky.social">bluesky</a></li>
    <li><i class='fas fa-paint-brush'></i> <a href="https://bloomzeye.nekoweb.org/">portfolio website</a> - (eventually, I'll transfer my portfolio to my blog here!)</li>
    <li><i class='fas fa-gamepad'></i> <a href="https://bloomzeye.itch.io/">my games on itch.io</a></li>
    <li><i class='fas fa-moon'></i> <a href="https://www.pillowfort.social/bloomzeye">pillowfort</a></li>
    <li><i class='fab fa-tumblr'></i> <a href="https://www.tumblr.com/blog/bloomzeye">tumblr</a></li>
    <li><i class='fab fa-youtube'></i> <a href="https://www.youtube.com/@bloomzeyestudio">youtube</a></li>
</ul>

